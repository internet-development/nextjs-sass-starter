...
async function onAuthenticate({ email, password }) {
  const response = await fetch('https://internet-apis.onrender.com/api/users/authenticate', {
    method: 'POST',
...
async function onRefreshAPIKey({ email, password }) {
  const response = await fetch('https://internet-apis.onrender.com/api/users/regenerate-key', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, password }),
  });
